...
const CompanionsList = async ({title, companions, classNames}: CompanionsListProps) => {
    const user = await currentUser();
    const usersId = companions?.filter(companion => user?.id === companion.author);

    return (
        <article className={cn("companions-list", classNames)}>
            <h2 className="font-bold text-3xl underline mb-4">{title}</h2>
            {usersId?.length ?
                (
                    <Table>
                        <TableHeader>
                        ...
                        <TableBody>
                            {companions?.map(({id, subject, name, topic, duration, author}) => (
                                user && user.id === author &&
                                    <TableRow key={id}>
                                        <TableCell>
